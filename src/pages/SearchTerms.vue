	...
	<section>
		...
		<form @submit.prevent="handleSubmit">
			<span class="p-input-icon-left" style="margin-bottom: 1rem">
				...
				<InputText class="input-text" type="text" v-model="searchQuery" placeholder="Search" style="width: 20rem" />
			</span>
			<Button
				v-if="!isLoading"
				style="color: white"
		...
		>
			...
			<template #footer>
				<Button class="p-button-secondary" label="Okay" @click="displayDialog" v-scroll-to="'#viewResult'" />
			</template>
		</Dialog>
		...
		<div id="viewResult">
			...
			<div v-else class="result-container" v-for="result in resultArray" :key="result">
				<div class="icon-container">
					<a :href="result.url" class="result-title" target="_blank"
						><h4>{{ result.title }}</h4></a
					>
					<a :href="result.url" target="_blank"><i class="external-link pi pi-external-link"></i></a>
				</div>
				...
				>
				<p class="result-snippet"><span v-html="result.snippet"></span>&nbsp;...</p>
			</div>
		</div>
	</section>
</template>
